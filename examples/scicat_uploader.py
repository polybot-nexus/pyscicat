...
class ScicatDataSetUploader(object):
    ...
    def check_dataset_catalog_status(self, scicat_client, extension=""):
        """Ensure we search for datasets using the correct PID format."""
        
        dataset_name = self.dataset_name.replace(extension, "")
        
        # ✅ Use full PID format
        search_pid = f"20.500.12269/{self.unique_id}"
        # print(f"🔍 Searching for dataset with PID: {search_pid}")
        
        found_datasets = scicat_client.datasets_find(query_fields={"pid": search_pid})
        
        # 🔍 Fallback: If not found, try searching by dataset name
        if not found_datasets:
            # print(f"🔄 Dataset not found by PID, trying dataset name: {dataset_name}")
            found_datasets = scicat_client.datasets_find(query_fields={"datasetName": dataset_name})

        # ✅ If dataset exists and overwrite is enabled, allow deletion
        if self.overwrite_data and found_datasets:
            self.delete_from_scicat = True
    ...
    def to_scicat(self, scicat_client): 
        ...
        #     os.system(f"del {self.unique_id}.json")  # Windows command
        # else:
        #     os.system(f"rm {self.unique_id}.json")  # Linux/macOS command


        os.system(f"del {self.unique_id}.json") 
    ...
    def to_scicat_updateonly(self, scicat_client): 
        """Update dataset in SciCat instead of creating a new one."""
        try:
            if self.instrument_name == 'bioglow_spec':
                self.metadata_dictionary['measurement']['bioglow_spec']['countrate_blocks'] = None
                self.metadata_dictionary['measurement']['bioglow_spec']['record_blocks'] = None
            ...
            print(f"⚠️ Error modifying metadata: {err}")
        ...
        print(f"📂 Google Drive Info: {self.google_drive_info}")
        colab_link_html = self.add_colab_links()
        gdrive_links = [
            f"{x['drive_name']}: {x['url']}"
            for x in self.google_drive_info if x['status'] == 'current' and 'Organized' in x['folder_path_in_drive']
        ]
        description_links = "<br>".join(gdrive_links) + "<br>" + colab_link_html

        validated_kw = [x for x in self.keywords if x not in ["list", "tags", "separated", "by", "commas", "(Optional)"]]

        new_dataset = RawDataset(
            sourceFolder=self.source_folder,
            owner=self.owner,
            datasetName=self.dataset_name,
            contactEmail=self.email,
        ...
        # ✅ Ensure we use the correct PID from SciCat
        cataloged = self.check_dataset_catalog_status(scicat_client)
        if not cataloged:
            print(f"❌ Error: Dataset '{self.unique_id}' not found in SciCat. Skipping update.")
            return "Dataset not found, skipping update."

        dataset_id = cataloged[0]['pid']  # Use the exact SciCat PID
        print(f"✅ Updating dataset with PID: {dataset_id}")

        try:
            ds_update_out = scicat_client.datasets_update(new_dataset, dataset_id)
            print(f"✅ Successfully updated dataset: {dataset_id}")
            return ds_update_out
    ...
    def upload_thumbnail_attachments(self, scicat_client):
        ...
        for x in self.thumbnails:
            attachment = Attachment(datasetId = dataset_id,
